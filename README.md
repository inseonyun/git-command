...
+ ### [특정 Commit 시점으로 돌아가기](#특정-commit-시점으로-돌아가기-1)
...
+ ## [클라우드 저장소 (원격 저장소) 내려받기](#클라우드-저장소-원격-저장소-내려받기)
...
+ ## [클라우드 저장소 (원격 저장소) 의 변경 사항 내려 받기](#클라우드-저장소-원격-저장소-의-변경-사항-내려-받기)
...
+ ## [특정 Commit 시점으로 돌아가기](#특정-commit-시점으로-돌아가기-1)
  + ### 협업을 하거나 개인 작업을 하며 기능 개발을 할 때, 특정 Commit으로 rollback 해야하는 경우가 종종 있다. 그 때 사용하기 유용하다. 단, 협업 시에는 revert 명령어를 주로 사용하고, reset의 경우 혼자 작업하거나 협업하는 Repo, 브렌치를 나만 사용하고 있을 때 사용한다.
  
    ### 1. git log 명령어로 내가 돌아갈 시점의 Commit ID 확인하기
    <pre>
      git log       // Commit 히스토리가 쫘악 나열됨.
    ...
    ### reset 전에 주의할 점은 해당 Repo를 나만 사용하는지, 그리고 변경 사항들이 git add 되어 추적이 가능한지!! (추적이 불가능 할 경우 추적 불가능한 변경 사항들은 남아있음)
    
    ### 2. git reset 명령어로 특정 Commit ID로 rollback
    <pre>
      git reset --hard [Commit ID]     // 해당 Commit ID 부분에 Checkin 할 Commit ID를 적는다
      
      <예시>
      git reset --hard a1b2c3
    </pre>
    
    ### 이후에는 뭐,,, 잘못 commit / push 된 게 있으면 push -f 명령어로 밀어버리거나 그냥 롤백이 목적이었으면 롤백 후 사용하면 된다.
